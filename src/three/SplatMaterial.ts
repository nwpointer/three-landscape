...
export default class SplatStandardMaterialImpl extends MeshStandardMaterial {
    ...
    onBeforeCompile(shader:any) {
        ...
        `
          ...
          ${computeNormal({ normalMaps: this._normalMaps.value, detailMaps: this._detailMaps.value })};
...
const computeDiffuse = ({ diffuseMaps = [], saturation = [], brightness = [] }) => {
    return diffuseMaps
        .filter((d) => d !== null && d !== undefined)
        .map((_, i) => {
            // base rgba values
...
const computeNormal = ({ normalMaps = [], detailMaps = [] }) => {
...
const sampler2d = (name:any, data:any) => (data && data.length ? `uniform sampler2D ${name}[${data.length}];` : "");
const float = (name:any, data:any) => (data && data.length ? `uniform float ${name}[10];` : "");

// utility functions
export function splatIndex(i:any) {
    return Math.floor(i / 4);
}
export function splatChannel(i:any) {
    return ["r", "g", "b", "a"][i % 4];
}