...
import utilityFunctions from './utilityFunctions.glsl.js';
...
function Terrain({
  ...
  return /*#__PURE__*/React.createElement(TerrainMesh, {
...
function TerrainMesh({
  ...
  const {
    width,
    height
  } = materialParameters.displacementMap.image;
...
function computeDiffuseByLayer(layers) {
  return layers.filter(l => l.diffuse).map(({
    diffuse
  }, i) => {
...
function computeNormalByLayer(layers) {
  return layers.filter(l => l.normal).map(({
    normal
  }, i) => {
...
export { Terrain, TerrainMesh, terrainMaterial };
